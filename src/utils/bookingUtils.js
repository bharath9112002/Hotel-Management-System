...
// Calendar day (local time) of an ISO timestamp, as 'YYYY-MM-DD'.
export function localDateOf(isoTimestamp) {
  return new Date(isoTimestamp).toLocaleDateString('en-CA')
}

export function matchesBookingSearch(booking, term) {
  if (!term) return true
  return [
    booking.id,
    booking.guestName,
...
// `from`/`to` are inclusive 'YYYY-MM-DD' bounds; either may be empty. For
// stay dates a booking matches when any day of it (arrival through departure)
// falls inside the range.
export function matchesDateRange(booking, field, from, to) {
  if (!from && !to) return true
  if (field === 'booked') {
    const booked = localDateOf(booking.createdAt)
    return (!from || booked >= from) && (!to || booked <= to)
  }
  return (!from || booking.checkOut >= from) && (!to || booking.checkIn <= to)
}
